...
/// <summary>
/// This script handles the spaceships functionality, including movement and shooting.
/// The spaceship needs any Collider2D attached that has the trigger flag checked, so it can be hit
...
public class SpaceShip : MonoBehaviour {
    ...
    void Update() {
        ...
        // don't run update if we are dead.
		if(isDead) return;
    ...
    void OnCollisionEnter2D(Collision2D col){
        ...
        if(col.gameObject.CompareTag("Asteroid")){
			...
			// load the active scene again, to restard the game. We use a Coroutine for a slight delay to see the explosion.
            ...
            // instead we set isDead=true and remove the renderer to "hide" the object while we reload.
            isDead = true;
